...
if is_service_enabled l2gw-plugin; then
    if [[ "$1" == "source" ]]; then
        ...
        if is_service_enabled q-svc; then
            ...
            echo "Configuring networking-l2gw. NETWORKING_L2W_SERVICE_DRIVER is: "
            echo $NETWORKING_L2GW_SERVICE_DRIVER
            if [ "$NETWORKING_L2GW_SERVICE_DRIVER" ]; then
                echo "Adding L2GW Service Driver to L2GW_PLUGIN_CONF_FILE:"
                echo $L2GW_PLUGIN_CONF_FILE
